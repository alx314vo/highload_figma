...
| **operations** | Apache Cassandra | **Архитектурное обоснование:** Критично низкая задержка записи. Cassandra позволяет писать локально в каждом регионе с `LOCAL_QUORUM`, что дает меньшую задержку при записи в удаленный мастер. Высокая нагрузка требует горизонтального масштабирования — Cassandra справляется лучше PostgreSQL. |
| **comments** | Apache Cassandra | **Архитектурное обоснование:** Аналогично operations — низкая задержка записи критична для UX. Комментарии могут появляться с небольшой задержкой репликации между регионами, что приемлемо. Большой объем данных и высокая нагрузка на запись требуют распределенной СУБД. |
