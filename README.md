...
1. **Low Latency как первый приоритет** — все решения направлены на минимизацию задержки для коллаборации в настоящем времени
2. **Распределенная запись** — операции записываются локально в каждом регионе для минимальной задержки
3. **Eventual Consistency где допустимо** — операции редактирования могут реплицироваться асинхронно между регионами
4. **Strong Consistency где критично** — метаданные файлов и права доступа требуют строгой консистентности
...
**Архитектурное требование: Минимизация задержек как первый приоритет**
...
*   **Для повышения вовлеченности:** Быстрая загрузка файлов и отклик интерфейса увеличивают время сессии и активность. Задержка загрузки файла >500 мс приводит к потере 15% пользователей.

**Архитектурное решение:** Распределение ДЦ по регионам с максимальной концентрацией пользователей (США 38%, Европа 6%+, Россия остальное) минимизирует сетевую задержку. Каждый регион обслуживается локально, что дает задержку <20 мс вместо 50-150 мс при централизованной архитектуре.
...
- **География пользователей четко разделена по регионам** — нет смысла балансировать между несколькими ДЦ в одном регионе, так как задержка между регионами велика (50-150 мс)
- **Geo-Based DNS обеспечивает достаточную гибкость** — позволяет направлять пользователей в ближайший ДЦ с возможностью fallback при отказе региона
- **WebSocket-соединения требуют стабильного endpoint** — Anycast может менять endpoint при изменении маршрутизации BGP, что разорвет WebSocket-соединения и нарушит коллаборацию
- **Локальная запись в Cassandra требует фиксированного endpoint** — для `LOCAL_QUORUM` нужно знать, какие ноды находятся в локальном регионе
...
Ключевое нефункциональное требование системы — **минимальная задержка (low latency)**. Для реалтайм-коллаборации критично, чтобы операции редактирования сохранялись и синхронизировались с минимальной задержкой. Поэтому:

- **Операции записи** (`POST /operation`, `POST /comment`) записываются в **ближайший региональный ДЦ** в Cassandra. Это ключевая фишка Cassandra — она позволяет писать локально с уровнем консистентности `LOCAL_QUORUM`, а затем асинхронно реплицировать данные в другие регионы. Альтернатива (запись только в Москву) добавила бы 50-150 мс задержки для пользователей из США/Европы, что неприемлемо для реалтайм-редактора.

- **Операции чтения** (`GET /file/{id}`, `GET /files`) обслуживаются из ближайшего ДЦ с кэшированием через CDN

- **WebSocket-соединения** устанавливаются в ближайшем ДЦ. Синхронизация операций происходит через региональный Collab_Service, который читает операции из локальной Cassandra и broadcastит их подключенным пользователям

- **PostgreSQL** (метаданные файлов, пользователи) использует мастер-репликацию с мастером в Москве, так как эти операции не критичны по задержке и требуют строгой консистентности
...
1. **L4-балансировщик** (Linux Virtual Server + Keepalived):
   - **Архитектурное обоснование:** LVS работает на сетевом уровне (Layer 4), не анализируя содержимое пакетов, что дает максимальную производительность (15,000 RPS на инстанс). Альтернатива (только NGINX L7) добавила бы overhead на анализ HTTP-заголовков и снизила бы пропускную способность. LVS распределяет трафик между L7-балансировщиками по алгоритму **Least Connections**, что оптимально для WebSocket-соединений с разной длительностью.
   - Обеспечивает отказоустойчивость через виртуальный IP (VRRP) — при отказе одного L4-балансировщика Keepalived автоматически переключает трафик на резервный
   - Формула резервирования: **N+1**

2. **CDN** (Cloudflare / AWS CloudFront):
   - **Архитектурное обоснование:** В системе много статики (JS/CSS бандлы, ассеты, превью изображений файлов). NGINX может не справиться с пиковыми нагрузками на статику, особенно при DDoS-атаках. CDN решает обе проблемы:
     - Разгружает NGINX от статики (до 70% трафика)
     - Защита от DDoS на уровне edge-нод CDN
...
3. **L7-балансировщик** (NGINX):
   ...
   - Кэширует метаданные файлов (не статику — она в CDN)
   - Формула резервирования: **N+1**
...
|---------|------|----------------|
| **users, teams, team_members, projects, files, file_versions, file_access** | PostgreSQL | **Архитектурное обоснование:** Реляционные данные требуют транзакций и сложных JOIN-запросов (например, проверка прав доступа с объединением file_access и users). PostgreSQL отлично справляется с такими запросами благодаря индексам и оптимизатору. Альтернатива (Cassandra) не подходит — нет JOIN'ов, сложные запросы неэффективны. |
| **operations** | Apache Cassandra | **Архитектурное обоснование:** Критично низкая задержка записи (low latency requirement). Cassandra позволяет писать локально в каждом регионе с `LOCAL_QUORUM`, что дает задержку <5 мс вместо 50-150 мс при записи в удаленный мастер. Асинхронная репликация между регионами обеспечивает eventual consistency, что приемлемо для операций редактирования. Высокая нагрузка (27,969 QPS) требует горизонтального масштабирования — Cassandra справляется лучше PostgreSQL. |
| **comments** | Apache Cassandra | **Архитектурное обоснование:** Аналогично operations — низкая задержка записи критична для UX. Комментарии могут появляться с небольшой задержкой репликации между регионами (eventual consistency), что приемлемо. Большой объем данных (8.3 ТБ) и высокая нагрузка на запись требуют распределенной СУБД. |
...
| **Cassandra** | **Multi-region replication:** Replication Factor = 3 в каждом регионе. **Архитектурное обоснование:** Запись происходит локально в ближайшем регионе с `LOCAL_QUORUM` (2 из 3 нод в регионе), что дает задержку <5 мс вместо 50-150 мс при записи в удаленный мастер. Асинхронная репликация между регионами обеспечивает низкую задержку записи при сохранении отказоустойчивости. При отказе одной ноды в регионе система продолжает работать (2 из 3 нод доступны). При полном отказе региона данные доступны из других регионов с небольшой задержкой репликации (eventual consistency). |
...
|----------|-------------------|-----------|
| **Operational Transform (OT)** | Реалтайм-коллаборация | **Архитектурное обоснование:** Обеспечивает консистентность при одновременном редактировании файла несколькими пользователями. Операции трансформируются относительно друг друга, что позволяет избежать конфликтов. Критично для low latency — операции применяются локально без ожидания подтверждения от сервера, что дает отклик <10 мс. Альтернатива (блокировки) добавила бы задержку 50-100 мс на каждую операцию, что неприемлемо для реалтайм-редактора. |
| **CRDT (Conflict-free Replicated Data Types)** | Синхронизация состояния редактора | **Архитектурное обоснование:** Альтернатива OT для некоторых типов операций (например, добавление элементов в список). Гарантирует конвергенцию состояния без центрального координатора, что идеально для распределенной архитектуры с локальной записью в каждом регионе. CRDT работает лучше OT для операций, где порядок не критичен, так как не требует трансформации операций. |
| **Differential Sync** | Синхронизация изменений файлов | **Архитектурное обоснование:** Передача только изменений (diff) вместо полного файла (8 МБ средний размер) снижает трафик в 10-100 раз. Это критично для low latency — передача diff занимает <50 мс вместо 500-1000 мс для полного файла. Альтернатива (передача полного файла) увеличила бы задержку загрузки и сетевой трафик, что неприемлемо при 447 RPS открытий файлов. |
| **Lazy Loading** | Загрузка элементов холста | **Архитектурное обоснование:** Загружаются только видимые элементы и элементы вблизи viewport, что ускоряет открытие файлов с 2-3 секунд до <500 мс. Критично для UX — пользователь видит файл почти мгновенно, остальные элементы подгружаются по мере необходимости. Альтернатива (загрузка всех элементов) увеличила бы время открытия файла в 5-10 раз, что неприемлемо для профессионального инструмента. |
...
| **WebSocket** | Реалтайм-коллаборация | **Архитектурное обоснование:** Двусторонняя связь критична для реалтайм-коллаборации. Альтернативы (HTTP long polling, Server-Sent Events) добавляют задержку из-за overhead на установку соединения. WebSocket устанавливается один раз и поддерживает низкую задержку передачи операций (<10 мс), что критично для синхронизации курсоров и изменений между пользователями. |
| **NGINX** | L7 балансировка нагрузки, прокси | **Архитектурное обоснование:** SSL-терминация, обратное проксирование, кэширование метаданных. Высокая производительность для обработки HTTP/WebSocket трафика (10,000 RPS на инстанс). Альтернативы (HAProxy, Envoy) либо менее производительны, либо избыточны по функционалу. NGINX — оптимальный баланс производительности и простоты конфигурации. Статика отдается через CDN, NGINX обрабатывает только динамические запросы. |
...
| **Kubernetes** | Оркестрация | **Архитектурное обоснование:** Автоматическое масштабирование сервисов критично для обработки пиковых нагрузок (99,903 RPS пик vs 33,301 RPS средний — разница в 3×). HPA позволяет масштабировать под нагрузкой без ручного вмешательства, что критично для SaaS с переменной нагрузкой. Альтернативы (Docker Swarm, ручное управление) требуют ручного масштабирования, что приводит к простоям при всплесках нагрузки или перерасходу ресурсов в спокойное время. |
| **PostgreSQL** | Основная база данных | **Архитектурное обоснование:** Реляционная СУБД с поддержкой транзакций, сложных запросов и индексов. Критично для метаданных файлов и пользователей, где нужны JOIN'ы (например, проверка прав доступа: file_access JOIN users). Альтернативы (MongoDB, Cassandra) не подходят — нет транзакций или сложных запросов. PostgreSQL обеспечивает ACID-гарантии, что критично для финансовых операций и прав доступа. |
| **Apache Cassandra** | Хранение операций и комментариев | **Архитектурное обоснование:** Ключевая фишка Cassandra — возможность локальной записи в каждом регионе с `LOCAL_QUORUM`, что критично для low latency requirement. Альтернатива (PostgreSQL с мастером в одном регионе) добавила бы 50-150 мс задержки для пользователей из других регионов, что неприемлемо для реалтайм-редактора. Горизонтальное масштабирование и высокая пропускная способность записи (27,969 QPS) делают Cassandra оптимальным выбором. |
| **CDN (Cloudflare / AWS CloudFront)** | Отдача статики и защита от DDoS | **Архитектурное обоснование:** Статика (JS/CSS бандлы, ассеты) составляет до 70% трафика. NGINX может не справиться с пиковыми нагрузками, особенно при DDoS. CDN решает обе проблемы: разгружает NGINX и обеспечивает защиту на уровне edge-нод. Кэширование на edge-серверах по всему миру снижает задержку загрузки для пользователей. |
| **Redis** | Кэш и сессии | **Архитектурное обоснование:** Быстрое хранилище в памяти для кэширования метаданных файлов и хранения активных сессий. Задержка <1 мс критична для проверки сессий на каждом запросе (500 QPS чтения). Альтернативы (Memcached, in-memory cache) либо не имеют персистентности, либо менее функциональны. Redis с TTL идеален для сессий — автоматическое удаление истекающих сессий без дополнительной логики. |
| **Ceph (S3)** | Хранение файлов | **Архитектурное обоснование:** S3-совместимое хранилище с горизонтальным масштабированием и отказоустойчивостью. Объем данных огромен (7.3 ПБ), требуется распределенное хранилище. Технология CRUSH автоматически перераспределяет данные при добавлении нод. Альтернативы (локальные диски, NFS) не масштабируются и создают single point of failure. Ceph обеспечивает репликацию 3× и автоматическое восстановление при отказе нод. |
...
#### **Открытие файла**
1. Client → **CDN** (статичные ассеты: JS/CSS) → отдача из edge-кэша (<10 мс задержка)
...
3. **Editor_Service** → **Cassandra (operations) в локальном регионе** → сохранение операции с `LOCAL_QUORUM` (задержка <5 мс)
...
| **PostgreSQL** | Метаданные (RO + RW, мастер в Москве) | 100 QPS/ядро | 18 ГБ/реплика |
