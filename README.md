...
| **Остальные** | Распределены по ближайшим ДЦ | 3,510,000 | 27% | По географии |

**Всего 3 ДЦ**, на каждый приходится не более 5M активных пользователей в месяц.

Для глобальной балансировки используется **GeoDNS**, чтобы пользователи направлялись на ближайший датацентр. **DNS Failover** позволяет автоматически переключать трафик на резервные ДЦ при отказе основного.
...
- Нет фиксированного "мастер-региона" у файла — пользователь всегда редактирует через ближайший ДЦ, даже если файл был создан в другом регионе
...
**GeoDNS для основного домена:**
- `figma.com` → балансируется по географии пользователя:
  - Пользователи из США → Нью-Йорк
  - Пользователи из Европы → Франкфурт
  - Пользователи из России и СНГ → Санкт-Петербург
  - Остальные → ближайший ДЦ по latency
...
- `static.figma.com`, `preview.figma.com` → CDN (Cloudflare)
  ...
  - Если объекта нет в кэше CDN, запрос идет на ближайший региональный ДЦ

**DNS Failover:**
- При отказе основного ДЦ региона автоматически переключается на резервный ДЦ из таблицы выше
...
**Открытие файла:**
1. Клиент идет на `figma.com` → попадает в ближайший региональный ДЦ через GeoDNS
2. Региональный `Editor_Collab_Service`:
   - Читает метаданные файла из локальной реплики PostgreSQL
   - Берет последний снапшот и тело документа из локального Ceph S3
   - Подгружает операции после снапшота из локальной Cassandra
3. Клиент восстанавливает состояние: `snapshot + operations` через CRDT
...
2. Ответ приходит с ближайшего edge-сервера CDN
3. Если объекта нет в кэше CDN, запрос идет на ближайший региональный ДЦ за оригиналом
