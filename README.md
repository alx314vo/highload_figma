...
**Допущения:**
- Один NGINX L7-инстанс способен обрабатывать **10,000 RPS** 
...
С учетом отказоустойчивости и распределения по 3 зонам доступности: **9 инстансов NGINX L7**.
...
| **operations** | Apache Cassandra | Критично низкая задержка записи. Cassandra позволяет писать локально в каждом регионе, что дает меньшую задержку при записи в удаленный мастер. Высокая нагрузка требует горизонтального масштабирования — Cassandra справляется лучше PostgreSQL. |
...
#### **Создание версии**
1. **Version_Service** → получение события о накоплении операций
